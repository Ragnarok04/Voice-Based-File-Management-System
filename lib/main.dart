...
class _FileManagerHomePageState extends State<FileManagerHomePage> {
  ...
  @override
  @override
  List<FileNode> searchInTree(List<FileNode> nodes, String query) {
    List<FileNode> results = [];

    for (var node in nodes) {
      if (node.isFolder) {
        // search in children
        results.addAll(searchInTree(node.children, query));
      } else if (node.name.toLowerCase().contains(query)) {
        results.add(node);
      }
    }
  ...
  List<FileNode> get searchedFiles {
    if (searchQuery.isEmpty || rootNode == null) return [];
    return searchInTree(rootNode!.children, searchQuery);
  }
  ...
  Future<String> pickAnyFile() async {
    ...
    return result.split(',').last; // Return only the Base64 part.
  }
  ...
  Future<Map<String, dynamic>?> _executeGitCommand(String text) async {
    final uri = Uri.parse('http://localhost:8000/execute-git');
    ...
    if (resp.statusCode == 200) {
      ...
      debugPrint('Git execution error: ${resp.body}');
      return null;
    }
  }
  ...
  void _showFileInfoDialog(Map<String, dynamic> file) {
    String fileTypeInfo = file['is_dir'] ? "Folder" : getFileType(file['name']);
    showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: Icon(
              isRenaming ? Icons.check : Icons.drive_file_rename_outline,
            ),
          ...
          Expanded(
            ...
            child: Container(
              ...
              child: isLoading || rootNode == null
                  ...
                  : (searchQuery.isNotEmpty && searchedFiles.isEmpty)
                  ? const Center(child: Text('No results found'))
                  : ListView(
                      padding: const EdgeInsets.symmetric(vertical: 8),
                      children: searchQuery.isNotEmpty
                          ? _buildNodes(searchedFiles)
                          : _buildNodes(rootNode!.children),
                    ),
            ),
          ),
        ],
